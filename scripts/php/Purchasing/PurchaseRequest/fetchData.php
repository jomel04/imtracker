<?php
    ...
    //QUERY
    $query = "SELECT purchasing.purchasingID, pr.refNo, users.lastName, users.firstName, expense_account.type AS ExpenseAccount, section.type AS Section, pr.purpose, pr.remarks AS PurchaseRequestRemarks, pr.cost, pr.chargeTo, purchasing.dateReceived, purchasing.receivedBy, purchasing.status, purchasing.poNo, purchasing.remarks AS PurchasingRemarks, purchasing.releaseDate, 5 * (DATEDIFF(NOW(), purchasing.dateReceived) DIV 7) + MID('0123455401234434012332340122123401101234000123450', 7 * WEEKDAY(purchasing.dateReceived) + WEEKDAY(NOW()) + 1, 1) AS Days, lead_time.leadTime AS LeadTime FROM pr INNER JOIN purchasing ON pr.purchasingID = purchasing.purchasingID INNER JOIN expense_account ON pr.expenseID = expense_account.expenseID INNER JOIN section ON pr.sectionID = section.sectionID INNER JOIN lead_time ON purchasing.leadTimeID = lead_time.leadTimeID INNER JOIN manager ON pr.managerID = manager.managerID INNER JOIN users ON pr.userID = users.userID INNER JOIN cctl ON pr.cctlID = cctl.cctlID INNER JOIN budget ON pr.budgetID = budget.budgetID WHERE pr.state = 'Active' AND manager.status = 'Approved' AND cctl.status = 'Approved' AND budget.status = 'Approved' AND purchasing.status != 'Delivered' AND purchasing.status != 'Installed' AND purchasing.status != 'Ordered'";
