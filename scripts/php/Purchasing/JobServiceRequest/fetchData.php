<?php
    ...
    //QUERY
    $query = "SELECT purchasing.purchasingID, jsr.refNo, users.lastName, users.firstName, expense_account.type AS ExpenseAccount, section.type AS Section, jsr.purpose, jsr.remarks AS JobServiceRequestRemarks, jsr.cost, jsr.chargeTo, purchasing.dateReceived, purchasing.receivedBy, purchasing.status, purchasing.poNo, purchasing.remarks AS PurchasingRemarks, purchasing.releaseDate, 5 * (DATEDIFF(NOW(), purchasing.dateReceived) DIV 7) + MID('0123455401234434012332340122123401101234000123450', 7 * WEEKDAY(purchasing.dateReceived) + WEEKDAY(NOW()) + 1, 1) AS Days, lead_time.leadTime AS LeadTime FROM jsr INNER JOIN purchasing ON jsr.purchasingID = purchasing.purchasingID INNER JOIN expense_account ON jsr.expenseID = expense_account.expenseID INNER JOIN section ON jsr.sectionID = section.sectionID INNER JOIN lead_time ON purchasing.leadTimeID = lead_time.leadTimeID INNER JOIN manager ON jsr.managerID = manager.managerID INNER JOIN users ON jsr.userID = users.userID INNER JOIN cctl ON jsr.cctlID = cctl.cctlID INNER JOIN budget ON jsr.budgetID = budget.budgetID WHERE jsr.state = 'Active' AND manager.status = 'Approved' AND cctl.status = 'Approved' AND budget.status = 'Approved' AND purchasing.status != 'Delivered' AND purchasing.status != 'Installed' AND purchasing.status != 'Ordered'";
