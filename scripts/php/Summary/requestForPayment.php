<?php
    ...
    //QUERY
    $query = "SELECT rfp.rfpID, rfp.dateCreated, rfp.dateEntered, banana_calendars.week_number, banana_calendars.period_number, rfp.status, expense_account.type AS ExpenseAccount, section.type AS Section, users.lastName, users.firstName, rfp.payee, rfp.purpose, rfp.remarks, rfp.cost FROM rfp INNER JOIN banana_calendars ON rfp.calID = banana_calendars.calID INNER JOIN expense_account ON rfp.expenseID = expense_account.expenseID INNER JOIN section ON rfp.sectionID = section.sectionID INNER JOIN users ON rfp.userID = users.userID";

    //For Search Bar
    if(!empty($_POST["search"]["value"])) {
        $query .= " AND user.lastName LIKE '%".$_POST['search']['value']."%' OR user.firstName LIKE '%".$_POST['search']['value']."%' OR banana_calendars.week_number LIKE '%".$_POST['search']['value']."%' OR banana_calendars.period_number LIKE '%".$_POST['search']['value']."%' OR rfp.status LIKE '%".$_POST['search']['value']."%' OR expense_account.type LIKE '%".$_POST['search']['value']."%' OR section.type LIKE '%".$_POST['search']['value']."%'";
    }
	...
	function fetchAllData() {
		$dbOperation = new DatabaseOperation();
		try {
			$stmt = $dbOperation->connect()->prepare("SELECT rfp.rfpID, rfp.dateCreated, rfp.dateEntered, banana_calendars.week_number, banana_calendars.period_number, rfp.status, expense_account.type AS ExpenseAccount, section.type AS Section, users.lastName, users.firstName, rfp.payee, rfp.purpose, rfp.remarks, rfp.cost FROM rfp INNER JOIN banana_calendars ON rfp.calID = banana_calendars.calID INNER JOIN expense_account ON rfp.expenseID = expense_account.expenseID INNER JOIN section ON rfp.sectionID = section.sectionID INNER JOIN users ON rfp.userID = users.userID");
			$stmt->execute();
    	...
    	foreach($result as $row) {
            $subArray = array();
             if(strpos($row['status'], 'Cancelled')) {
                ...
                $subArray[] = '<div class="text-center" style="padding: 0 55px;color:tomato;">' . $row['status'] . '</div>';
                ...
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['week_number'] . '</div>';
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['period_number'] . '</div>';
                $subArray[] = '<div class="text-center" style="padding: 0 35px;color:tomato;">' . $row['status'] . '</div>';
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['ExpenseAccount'] . '</div>';
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['Section'] . '</div>';
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['lastName'] . ', ' . $row['firstName'] . '</div>';
                $subArray[] = '<div class="text-center" style="color:tomato;">' . $row['payee'] . '</div>';
                ...
                $subArray[] = '<div class="text-center" style="padding: 0 35px;">' . $row['status'] . '</div>';
