<?php
    ...
    //QUERY
    $query = "SELECT jsr.jsrID, jsr.dateCreated, jsr.dateEntered, banana_calendars.week_number, banana_calendars.period_number, jsr.refNo, jsr.status, expense_account.type AS ExpenseAccount, section.type AS Section, users.lastName, users.firstName, jsr.purpose, jsr.remarks, jsr.cost, jsr.chargeTo FROM jsr INNER JOIN banana_calendars ON jsr.calID = banana_calendars.calID INNER JOIN expense_account ON jsr.expenseID = expense_account.expenseID INNER JOIN section ON jsr.sectionID = section.sectionID INNER JOIN users ON jsr.userID = users.userID WHERE users.userID = " . $_SESSION['user'];

    //For Search Bar
    if(!empty($_POST["search"]["value"])) {
        $query .= " AND expense_account.type LIKE '%".$_POST['search']['value']."%' OR jsr.refNo LIKE '%".$_POST['search']['value']."%' OR banana_calendars.week_number LIKE '%".$_POST['search']['value']."%' OR banana_calendars.period_number LIKE '%".$_POST['search']['value']."%' OR jsr.chargeTo LIKE '%".$_POST['search']['value']."%'";
    }
