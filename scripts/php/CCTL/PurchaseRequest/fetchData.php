<?php
    ...
    //QUERY
    $query = "SELECT cctl.cctlID, pr.refNo, users.lastName, users.firstName, expense_account.type AS ExpenseAccount, section.type AS Section, pr.purpose, pr.remarks AS PurchaseRequestRemarks, pr.cost, pr.chargeTo, cctl.dateReceived, cctl.receivedBy, cctl.status, cctl.remarks AS CctlRemarks, cctl.dateApproved, 5 * (DATEDIFF(NOW(), cctl.dateReceived) DIV 7) + MID('0123455401234434012332340122123401101234000123450', 7 * WEEKDAY(cctl.dateReceived) + WEEKDAY(NOW()) + 1, 1) AS LeadTime FROM pr INNER JOIN cctl ON pr.cctlID = cctl.cctlID INNER JOIN expense_account ON pr.expenseID = expense_account.expenseID INNER JOIN section ON pr.sectionID = section.sectionID INNER JOIN lead_time ON cctl.leadTimeID = lead_time.leadTimeID INNER JOIN manager ON pr.managerID = manager.managerID INNER JOIN users ON pr.userID = users.userID WHERE pr.state = 'Active' AND manager.status = 'Approved' AND cctl.status != 'Approved'";
