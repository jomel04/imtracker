$(document).ready(function () {
    ...
    var dataTable = $('#accounting').DataTable({
        ...
        "order": [],
        "ajax": {
            url: "../scripts/php/Accounting/RequestForPayment/fetchData.php",
            type: "POST"
        },
        "columnDefs": [{
            "targets": [0],
    ...
    $(document).on('click', 'button[name="btnUpdateAccounting"]', function () {
        var id = $(this).attr('id');
        $.ajax({
            url: '../scripts/php/Accounting/RequestForPayment/selectData.php',
            method: "POST",
            data: {
                id: id
            },
    ...
    $(document).on('click', 'button[name="btnSubmitAccounting"]', function () {
        ...
        if (dateReceivedAccounting != "" && receivedByAccounting != "" && statusAccounting != "" && releaseDateAccounting != "") {
            $.ajax({
                url: "../scripts/php/Accounting/RequestForPayment/updateData.php",
                method: "POST",
                data: {
                    id: id,
    ...
    $(document).on("click", "button[name='btnDeleteAccounting']", function () {
        ...
        if (confirm('Are you sure you want to remove this data?')) {
            $.ajax({
                url: "../scripts/php/Accounting/RequestForPayment/deleteData.php",
                method: "POST",
                data: {
                    id: id
                },
