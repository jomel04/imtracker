$(document).ready(function () {
    ...
    var dataTable = $('#budget').DataTable({
        ...
        "order": [],
        "ajax": {
            url: "../scripts/php/Budget/JobServiceRequest/fetchData.php",
            method: "POST"
        },
        "columnDefs": [{
            "targets": [0],
    ...
    $(document).on('click', 'button[name="btnUpdateBudget"]', function () {
        var id = $(this).attr('id');
        $.ajax({
            url: '../scripts/php/Budget/JobServiceRequest/selectData.php',
            method: "POST",
            data: {
                id: id
            },
    ...
    $(document).on('click', 'button[name="btnSubmitBudget"]', function () {
        ...
        if (budgeted != "" && dateReceivedBudget != "" && dateApprovedBudget != "" && receivedByBudget != "" && statusBudget != "") {
            $.ajax({
                url: "../scripts/php/Budget/JobServiceRequest/updateData.php",
                method: "POST",
                data: {
                    id: id,
                    budgeted: budgeted,
    ...
    $(document).on("click", "button[name='btnDeleteBudget']", function () {
        ...
        if (confirm('Are you sure you want to remove this data?')) {
            $.ajax({
                url: "../scripts/php/Budget/JobServiceRequest/deleteData.php",
                method: "POST",
                data: {
                    id: id
                },
