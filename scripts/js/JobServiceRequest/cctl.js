$(document).ready(function () {
    ...
    var dataTable = $('#cctl').DataTable({
        ...
        "order": [],
        "ajax": {
            url: "../scripts/php/CCTL/JobServiceRequest/fetchData.php",
            method: "POST"
        },
        "columnDefs": [{
            "targets": [0],
    ...
    $(document).on('click', 'button[name="btnUpdateCctl"]', function () {
        var id = $(this).attr('id');
        $.ajax({
            url: '../scripts/php/CCTL/JobServiceRequest/selectData.php',
            method: "POST",
            data: {
                id: id
            },
    ...
    $(document).on('click', 'button[name="btnSubmitCctl"]', function () {
        ...
        if (dateReceivedCctl != "" && receivedByCctl != "" && statusCctl != "" && dateApprovedCctl != "") {
            $.ajax({
                url: "../scripts/php/CCTL/JobServiceRequest/updateData.php",
                method: "POST",
                data: {
                    id: id,
    ...
    $(document).on("click", "button[name='btnDeleteCctl']", function () {
        ...
        if (confirm('Are you sure you want to remove this data?')) {
            $.ajax({
                url: "../scripts/php/CCTL/JobServiceRequest/deleteData.php",
                method: "POST",
                data: {
                    id: id
                },
    ...
    $(document).on('click', '#btnSkip', function () {
        ...
        if (confirm('Are you sure you want to skip it?')) {
            $.ajax({
                url: "../scripts/php/CCTL/JobServiceRequest/skip.php",
                method: "POST",
                data: {
                    id: id
                },
